...
TEST_CASE("fire_spread_cap_is_global_across_dimensions",
          "[simulation][field][fire_spread][dimension]") {
    clear_all_state();
    put_player_underground();
...
TEST_CASE("stable_load_requests_are_distinguished_from_propagating_requests",
...
TEST_CASE("load_request_source_stability_is_explicit",
          "[simulation][submap_loading]") {
    const auto stable_sources = std::array {
        load_request_source::reality_bubble,
        load_request_source::power_portal,
        load_request_source::player_claim,
        load_request_source::player_priority,
        load_request_source::stable_lua,
    };
    const auto unstable_sources = std::array {
        load_request_source::unstable_lua,
        load_request_source::fire_spread,
        load_request_source::vehicle_footprint,
        load_request_source::lazy_border,
        load_request_source::portal_preload,
    };

    for( const auto source : stable_sources ) {
        CHECK( load_request_source_is_stable( source ) );
    }
...
TEST_CASE("stable_contact_promotes_complete_vehicle_footprint",
          "[simulation][submap_loading][vehicle]") {
    clear_all_state();
    put_player_underground();

    const auto vehicle_sm = point_abs_sm{ 200, 200 };
    const auto vehicle_pos = project_to<coords::ms>( tripoint_abs_sm{ vehicle_sm, 0 } ) +
    ...
    auto *vehicle = MAPBUFFER.add_vehicle( vproto_id( "car" ), vehicle_pos, 0_degrees, 0, 0,
                                           true, std::nullopt, std::nullopt, resident_lookup );
    ...
    for( const auto &footprint : footprints ) {
        ...
        for( const auto &pos : point_range<point_abs_sm>( footprint->min.xy(),
                footprint->max.xy() ) ) {
    ...
    const auto non_stable_cell = std::ranges::find_if( footprint_cells,
                                  [&]( const point_abs_sm &pos ) {
                                      return pos != vehicle_sm;
                                  } );
    REQUIRE( non_stable_cell != footprint_cells.end() );

    auto fire_handle = submap_loader.request_load(
                             load_request_source::fire_spread, MAPBUFFER.get_dimension_id(),
                             vehicle_sm, vehicle_sm + point_rel_sm{ 1, 1 } );
    auto stable_handle = load_request_handle{};
    const auto cleanup = on_out_of_scope( [&]() {
        submap_loader.flush_prev_desired();
        if( vehicle != nullptr ) {
            MAPBUFFER.destroy_vehicle( vehicle, resident_lookup );
        }
    ...
    CHECK_FALSE( submap_loader.is_stably_requested( MAPBUFFER.get_dimension_id(),
                  *non_stable_cell ) );
    CHECK_FALSE( submap_loader.is_requested( MAPBUFFER.get_dimension_id(), *non_stable_cell ) );

    stable_handle = submap_loader.request_load(
                             load_request_source::stable_lua, MAPBUFFER.get_dimension_id(),
                             vehicle_sm, vehicle_sm + point_rel_sm{ 1, 1 } );
...
TEST_CASE("vehicle_footprints_do_not_chain",
          "[simulation][submap_loading][vehicle]") {
    clear_all_state();
    put_player_underground();
    ...
    const auto discard_vehicle = [&]( vehicle *&veh ) {
        ...
        const auto is_owned = owner != nullptr && std::ranges::any_of( owner->vehicles,
    ...
    first_vehicle = MAPBUFFER.add_vehicle( vproto_id( "car" ), first_vehicle_pos,
                         0_degrees, 0, 0, true, std::nullopt, std::nullopt, resident_lookup );
    second_vehicle = MAPBUFFER.add_vehicle( vproto_id( "car" ), second_vehicle_pos,
                          0_degrees, 0, 0, true, std::nullopt, std::nullopt, resident_lookup );
    ...
    const auto second_contact_cell = std::ranges::find_if( second_cells,
                                     [&]( const point_abs_sm &pos ) {
                                         return submap_loader.is_requested(
                                                    MAPBUFFER.get_dimension_id(), pos );
                                     } );
    const auto second_unpromoted_cell = std::ranges::find_if( second_cells,
                                           [&]( const point_abs_sm &pos ) {
                                               return !submap_loader.is_requested(
                                                          MAPBUFFER.get_dimension_id(), pos );
                                           } );
    ...
    CHECK_FALSE( submap_loader.is_requested( MAPBUFFER.get_dimension_id(),
                  *second_contact_cell ) );
    CHECK_FALSE( submap_loader.is_requested( MAPBUFFER.get_dimension_id(),
                  *second_unpromoted_cell ) );
}

TEST_CASE("vehicle_footprint_promotion_can_be_disabled",
          "[simulation][submap_loading][vehicle]") {
    clear_all_state();
    put_player_underground();
...
TEST_CASE("vehicle_footprint_promotion_covers_multiple_zlevels",
          "[simulation][submap_loading][vehicle]") {
    clear_all_state();
    put_player_underground();
    ...
    auto *vehicle = MAPBUFFER.add_vehicle( vproto_id( "car" ), vehicle_pos, 0_degrees, 0, 0,
                                           true, std::nullopt, std::nullopt, resident_lookup );
    REQUIRE( vehicle != nullptr );
    REQUIRE( vehicle->install_part( tripoint_mnt_veh{ 0, 0, 1 },
                                    vpart_id( "frame_vertical" ), true ) >= 0 );
    ...
    CHECK( MAPBUFFER.is_column_state( upper_footprint->min.xy(),
