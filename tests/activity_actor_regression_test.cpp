...
class lifecycle_actor final : public activity_actor {
    ...
    public:
        explicit lifecycle_actor( std::shared_ptr<lifecycle_state> state_in,
                                  bool seed_progress_in = true,
                                  bool stop_without_progress_in = true ) :
            state( std::move( state_in ) ), seed_progress( seed_progress_in ),
        ...
        void start( player_activity &, Character & ) override {
            state->starts++;
            if( seed_progress ) {
                progress.emplace( "activity actor regression", 100 );
            }
        }

        void do_turn( player_activity &act, Character & ) override {
            state->turns++;
            if( !seed_progress && stop_without_progress ) {
                act.set_to_null();
            }
        }
...
TEST_CASE( "shared actor ids retain their dispatch registrations",
...
TEST_CASE( "activity actor owns lifecycle and progress", "[activity][activity_actor]" ) {
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<lifecycle_actor>( state ) ) );
...
TEST_CASE( "actor without generic progress can end safely", "[activity][activity_actor]" ) {
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<lifecycle_actor>( state, false ) ) );
    REQUIRE( dummy.activity );
...
TEST_CASE( "actor without progress retains legacy duration during migration",
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<lifecycle_actor>( state, false, false ) ) );
    REQUIRE( dummy.activity );
...
TEST_CASE( "activity actor serialization preserves dispatch and progress",
...
TEST_CASE( "legacy activity actors reconstruct their own duration",
           "[activity][activity_actor][save_load][legacy]" ) {
    const auto wait_npc = deserialize_legacy_actor( activity_id( "ACT_WAIT_NPC" ),
    ...
    const auto train = deserialize_legacy_actor( activity_id( "ACT_TRAIN" ),
    ...
    const auto train_pet = deserialize_legacy_actor( activity_id( "ACT_TRAIN_PET" ),
    ...
    const auto wear = deserialize_legacy_actor( activity_id( "ACT_WEAR" ),
...
TEST_CASE( "simple actor activities complete through actor-owned progress",
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<wait_npc_actor>( "test NPC" ) ) );
    REQUIRE( dummy.activity );
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<train_actor>( "melee" ) ) );
    REQUIRE( dummy.activity );
...
TEST_CASE( "pet training cancels safely when its target is lost",
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<train_pet_actor>(
                                   weak_ptr_fast<monster>(), "lost pet" ) ) );
    REQUIRE( dummy.activity );
...
TEST_CASE( "wear actor cancels safely when every target is lost",
...
TEST_CASE( "wear actor completes a valid worn-item target",
...
TEST_CASE( "stamina wait owns an indefinite actor progress task",
           "[activity][activity_actor][progress]" ) {
    clear_map();
    clear_avatar();
    avatar &dummy = get_avatar();
...
TEST_CASE( "salvage actor cancels without targets",
           "[activity][activity_actor][salvage]" ) {
    clear_map();
    clear_avatar();
    avatar &dummy = get_avatar();
...
TEST_CASE( "ownerless active thrown items use the supplied absolute position",
...
TEST_CASE( "read activity progress message tolerates a lost book",
...
TEST_CASE( "butcher actor owns progress after corpse setup",
           "[activity][activity_actor][butcher]" ) {
    clear_map();
    clear_avatar();
    avatar &dummy = get_avatar();
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<butcher_actor>( activity_id( "ACT_BUTCHER" ),
                                       safe_reference<item>( corpse_ptr ) ) ) );
    REQUIRE( dummy.activity );
...
TEST_CASE( "butcher actor cancels when no suitable tool exists",
           "[activity][activity_actor][butcher]" ) {
    clear_map();
    clear_avatar();
    avatar &dummy = get_avatar();
...
TEST_CASE( "butcher actor completes and removes its corpse",
    ...
    CHECK_FALSE( buffer.add_item_or_charges( target,
                   item::make_corpse( mtype_id( "mon_zombie" ), calendar::turn, "" ) ) );
    auto *const items = buffer.get_items( target );
    REQUIRE( items != nullptr );
    REQUIRE( items->size() == 1 );
    item *const corpse_ptr = items->front();
...
TEST_CASE( "butcher actor cancels after its corpse is removed",
...
TEST_CASE( "shared butcher actors cancel when their corpse is removed",
    ...
    for( const activity_id &id : butcher_ids ) {
        INFO( "activity id: " << id.str() );
        REQUIRE_FALSE( buffer.add_item_or_charges( target,
                           item::make_corpse( mtype_id( "mon_zombie" ), calendar::turn, "" ) ) );
        auto *const items = buffer.get_items( target );
        REQUIRE( items != nullptr );
        REQUIRE( items->size() == 1 );
        ...
        dummy.assign_activity( std::make_unique<player_activity>(
                                   std::make_unique<butcher_actor>( id,
                                           safe_reference<item>( corpse_ptr ) ) ) );
        REQUIRE( dummy.activity );
...
TEST_CASE( "liquid transfer cancels after its vehicle source is lost",
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<liquid_transfer_actor>(
                                   LST_VEHICLE, source, 0, LTT_MAP, dummy.abs_pos(),
                                   safe_reference<item>() ) ) );
    REQUIRE( dummy.activity );
...
TEST_CASE( "vehicle actors cancel after their vehicle is lost",
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<hotwire_car_actor>( target, 1, 1 ) ) );
    REQUIRE( dummy.activity );
    dummy.moves = dummy.get_speed();
...
TEST_CASE( "gunmod actor completes and cancels after a target is lost",
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<gunmod_add_actor>(
                                   100, 0, 0, "", safe_reference<item>( &gun ),
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<gunmod_add_actor>(
                                   100, 0, 0, "", safe_reference<item>( &second_gun ),
                                   safe_reference<item>( &second_mod ) ) ) );
    REQUIRE( dummy.activity );
    dummy.activity->get_actor()->progress.emplace( "installing gunmod", 1 );
...
TEST_CASE( "hand crank actor cancels after its tool is lost",
...
TEST_CASE( "start fire actor cancels after its tool is lost",
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<start_fire_actor>(
                                   0, dummy.abs_pos(), 0 ) ) );
    REQUIRE( dummy.activity );
...
TEST_CASE( "make zlave actor keeps its absolute corpse target",
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<make_zlave_actor>(
...
TEST_CASE( "pulp actor reads and mutates its absolute corpse target",
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<pulp_actor>( target ) ) );
    REQUIRE( dummy.activity );
...
TEST_CASE( "play with pet actor cancels after its pet is lost",
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<play_with_pet_actor>(
...
TEST_CASE( "milk actor cancels after its animal is lost",
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<milk_actor>(
                                   dummy.abs_pos() + tripoint_east ) ) );
    REQUIRE( dummy.activity );
...
TEST_CASE( "crowbar honors an explicit absolute target",
           "[activity][item_action][prying][target]" ) {
    clear_map();
    clear_avatar();
    avatar &dummy = get_avatar();
    mapbuffer &buffer = dummy.get_mapbuffer();
...
TEST_CASE( "first aid actor cancels after its target is lost",
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<firstaid_actor>( "torso" ) ) );
    REQUIRE( dummy.activity );
    REQUIRE( dummy.activity->has_actor() );
...
TEST_CASE( "churn activity completes at its absolute placement",
           "[activity][activity_legacy][churn]" ) {
    clear_map();
    clear_avatar();
    avatar &dummy = get_avatar();
    mapbuffer &buffer = dummy.get_mapbuffer();
...
TEST_CASE( "clear rubble actor completes at its absolute placement",
    ...
    dummy.assign_activity( std::make_unique<player_activity>(
                               std::make_unique<clear_rubble_actor>( target ) ) );
    REQUIRE( dummy.activity );
...
TEST_CASE( "spellcasting activity finishes its self-target cast",
...
TEST_CASE( "multiple butcher activity continues through its corpse backlog",
    ...
    zone_manager::get_manager().add( "test corpse zone", zone_type_id( "LOOT_CORPSE" ),
                                     faction_id( "your_followers" ), false, true,
                                     first_target, second_target );
    buffer.add_item_or_charges( first_target,
                                item::make_corpse( mtype_id( "mon_rabbit" ), calendar::turn, "" ) );
    buffer.add_item_or_charges( second_target,
                                item::make_corpse( mtype_id( "mon_rabbit" ), calendar::turn, "" ) );
