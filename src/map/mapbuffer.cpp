    ...
    const std::unordered_set<point_abs_sm> &columns )
{
    ...
    const bool simulation_set_changed =
        std::ranges::any_of( column_states_, [&columns]( const auto &entry ) {
        return ( entry.second == submap_column_load_state::simulated ) != columns.contains( entry.first );
    ...
    if( simulation_set_changed ) {
        for( auto &island_sounds : island_sounds_ ) {
            for( auto &sound : island_sounds ) {
                queued_sounds.push_back( std::move( sound ) );
            }
        }
    }
