...
class unpack_actor : public iuse_actor
{
    public:
        ...
        void load( const JsonObject &obj ) override;
        int use( Character &p, item &it, bool, const tripoint_abs_ms & ) const override;
        std::unique_ptr<iuse_actor> clone() const override;
...
class explosion_iuse : public iuse_actor
{
    public:
        ...
        void trigger_explosion( mapbuffer &here, const tripoint_abs_ms &p, Creature *source ) const;
};
...
class set_transformed_iuse : public iuse_transform
{
    public:
        ...
        int bypass( Character &p, item &it, bool t, const tripoint_abs_ms &pos ) const;
...
class reveal_map_actor : public iuse_actor
{
    public:
        ...
        */
        ...
        void show_revealed( Character &plr, item &, const tripoint_abs_omt &map ) const;
...
class firestarter_actor : public iuse_actor
{
    public:
        ...
        static bool prep_firestarter_use( Character &p, const tripoint_abs_ms &pos );
        /** Player here isn't const because pyromaniacs gain a mood boost from it */
        static void resolve_firestarter_use( Character &p, const tripoint_abs_ms &pos );
        /** Modifier on speed - higher is better, 0 means it won't work. */
        float light_mod( const Character &p, const tripoint_abs_ms &pos ) const;
        /** Checks quality of fuel on the tile and interpolates move cost based on that. */
        int moves_cost_by_fuel( Character &p, const tripoint_abs_ms &pos ) const;
...
class learn_spell_actor : public iuse_actor
{
    public:
        ...
        void load( const JsonObject &obj ) override;
        int use( Character &p, item &, bool, const tripoint_abs_ms & ) const override;
        std::unique_ptr<iuse_actor> clone() const override;
        void info( const item &, std::vector<iteminfo> & ) const override;
};
...
class holster_actor : public iuse_actor
{
    public:
        ...
        detached_ptr<item> store( Character &p, item &holster, detached_ptr<item> &&obj ) const;
...
class bandolier_actor : public iuse_actor
{
    public:
        ...
        bool reload( Character &p, item &obj ) const;
...
class repair_item_actor : public iuse_actor
{
    public:
        ...
        attempt_hint repair( Character &pl, item &tool, item &fix ) const;
        ...
        bool can_repair_target( Character &pl, item &fix, bool print_msg ) const;
        /** Checks if we are allowed to use the tool. */
        bool can_use_tool( const Character &p, item &tool, bool print_msg ) const;
        ...
        bool handle_components( Character &pl, item &fix, bool print_msg, bool just_check ) const;
...
class place_trap_actor : public iuse_actor
{
    public:
        ...
        bool is_allowed( Character &p, const tripoint_abs_ms &pos, const std::string &name ) const;

        place_trap_actor( const std::string &type = "place_trap" );
        ~place_trap_actor() override = default;
        void load( const JsonObject &obj ) override;
        int use( Character &, item &, bool, const tripoint_abs_ms &pos ) const override;
        std::unique_ptr<iuse_actor> clone() const override;
};
...
class saw_barrel_actor : public iuse_actor
{
    public:
        ...
        int use( Character &p, item &it, bool t, const tripoint_abs_ms &pnt ) const override;
        std::unique_ptr<iuse_actor> clone() const override;

        ret_val<bool> can_use_on( const Character &p, const item &it, const item &target ) const;
};
...
class deploy_tent_actor : public iuse_actor
{
    public:
        ...
        bool check_intact( Character &, const tripoint_abs_ms &center ) const;
};
...
class weigh_self_actor : public iuse_actor
{
    public:
        ...
        void load( const JsonObject &jo ) override;
        int use( Character &p, item &, bool, const tripoint_abs_ms &pos ) const override;
        std::unique_ptr<iuse_actor> clone() const override;
        void info( const item &, std::vector<iteminfo> & ) const override;
};
...
class hand_crank_actor : public iuse_actor
{
    public:
        ...
        auto use( Character &p, item &it, bool, const tripoint_abs_ms & ) const -> int override;
        auto can_use( const Character &, const item &, bool,
                      const tripoint_abs_ms & ) const -> ret_val<bool> override;
...
class iuse_flowerpot_plant final : public iuse_actor
{
    ...
    public:
        ...
        void load( const JsonObject &jo ) override;
        int use( Character &who, item &i, bool, const tripoint_abs_ms & ) const override;
        ret_val<bool> can_use( const Character &, const item &, bool,
                               const tripoint_abs_ms & ) const override;
        void info( const item &, std::vector<iteminfo> & ) const override;
        std::unique_ptr<iuse_actor> clone() const override;
    private:
        ...
        static auto query_adjacent_pot( const Character &, bool empty = true ) -> std::optional<item *>;
        ...
        auto on_use_add_fertilizer( Character &, item & ) const -> int;
        auto on_use_plant( Character &, item & ) const -> int;
        auto on_use_harvest( Character &, item & ) const -> int;
        auto on_tick( Character &, item & ) const -> int;
...
class iuse_flowerpot_collect final : public iuse_actor
{
    ...
    public:
        ...
        void load( const JsonObject &obj ) override;
        auto use( Character &who, item &i, bool, const tripoint_abs_ms & ) const  -> int override;
        auto can_use( const Character &, const item &, bool,
                      const tripoint_abs_ms & ) const -> ret_val<bool> override;
        auto clone() const -> std::unique_ptr<iuse_actor> override;
    private:
        static void transfer_map_to_flowerpot( tripoint_abs_ms &map_pos,
                                               item &flowerpot,
...
class iuse_dimension_travel : public iuse_actor
{
    public:
        ...
        std::unique_ptr<iuse_actor> clone() const override;
    private:
        void dimension_travel( Character &p, item &, const tripoint_abs_ms &pos ) const;
};
...
class iuse_pocket_dimension : public iuse_actor
{
    ...
    private:
        ...
        void enter_pocket( Character &p, item &it ) const;
        void exit_pocket( Character &p, item &it ) const;
};
...
class iuse_portal_link : public iuse_actor
{
    public:
        ...
        void load( const JsonObject &obj ) override;
        auto use( Character &p, item &it, bool, const tripoint_abs_ms &pos ) const -> int override;
        auto can_use( const Character &, const item &it, bool,
                      const tripoint_abs_ms &pos ) const -> ret_val<bool> override;
        auto clone() const -> std::unique_ptr<iuse_actor> override;
};
...
class iuse_paint_stuff_config : public iuse_actor
{
    ...
    public:
        ...
        auto can_use( const Character &, const item &, bool,
                      const tripoint_abs_ms & ) const -> ret_val<bool> override;
        auto clone() const -> std::unique_ptr<iuse_actor> override;
        void on_placed( item & ) const override;
...
class iuse_paint_stuff : public iuse_actor
{
    ...
    public:
        ...
        void load( const JsonObject &obj ) override;
        auto use( Character &, item &, bool, const tripoint_abs_ms & ) const  -> int override;
        auto can_use( const Character &, const item &, bool,
                      const tripoint_abs_ms & ) const -> ret_val<bool> override;
        auto clone() const -> std::unique_ptr<iuse_actor> override;
        void info( const item &, std::vector<iteminfo> & ) const override;
